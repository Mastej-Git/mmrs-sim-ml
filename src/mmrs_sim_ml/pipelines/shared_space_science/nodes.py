...
def create_paths(
    agvs: list[dict],
    rand_ms_dict: dict[str, dict[str, list[tuple[float, float]]]]
    ) -> dict[str, dict[str, list[tuple[float, float]]]]:
    paths_dict = {}
    for name, rand_ms in rand_ms_dict.items():
        tmp = {}
        for agv_id, agv_ms_list in rand_ms.items():
            agv = agvs[int(agv_id[-1])]
            tmp[agv_id] = _create_path(agv_ms_list, agv["orientation"], agv["radius"])
        paths_dict[name] = tmp
